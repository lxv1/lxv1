...
function	_Field_isInteger(commas)
{
	...
	if	(commas)
	{	//	Strip commas, if they're allowed.
		sVal = sVal.split(",").join("");
	}

	//	Now parse it into a float.
	if( !_isLength(sVal, sVal.length, "numeric") ){
		this.error = "The value for " + this.description + " is not an integer value. This field requires an integer value.";
	}
}
